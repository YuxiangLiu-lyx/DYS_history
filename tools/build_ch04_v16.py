#!/usr/bin/env python3
"""Export identity-master plus text-motion prompts; preserve the locked audio and completed scenes."""
...
BASE=ROOT/'archive/chapter04_before_seedance_inference_v16'
...
def validate():
 ...
 for i,b in enumerate(t['blocks'][3:],3):
  ...
  assert b['image_reference_ids']==['C01','C05','V15_SMILE','S15_NEW','P_RUAN']
  assert b['static_kiss_reference_required'] is False
  ...
  if b['id'] in ['H04','H05']:
   ...
   assert b['prompt_status']=='prompt_ready_awaiting_actual_previous_clip'
   assert '缺失' not in p and '安全审核' not in p
 ...
 assert '轻拥' in t['blocks'][7]['last_frame_description']
 assert '相拥亲吻' in t['blocks'][8]['last_frame_description']
...
def build(out,selection=None):
 ...
 manifest=[];results=[];ordered=['# 第四回 v1.6 · 每段独立文件','','H01/H02/H03冻结。演唱两包保留真实音频及待核听材料；H04/H05正式口型Prompt尚未签发，不能直接提交。歌后四包含可审阅Prompt。','']
 for b in t['blocks'][3:]:
  bid=b['id'];song=bid in ['H04','H05'];m={'revision':t['revision'],'id':bid,'name':b['name'],'duration_seconds':b['duration'],'global_start':b['global_start'],'song_range':b.get('song_range'),'prompt_status':b['prompt_status'],'production_ready':False,'images':[],'audio':[],'previous_video':b['previous_reference'],'actual_video_review':False,'static_kiss_reference_required':False,'visual_route':'identity_masters_plus_prompt_motion'}
  for kind,ids,folder in [('images',b['image_reference_ids'],'02_images'),('audio',am[bid],'03_audio')]:
   for n,k in enumerate(ids,1):
    src=ROOT/a['refs'][k];m[kind].append({'slot':n,'ref_id':k,'source_path':a['refs'][k],'bundle_path':f'{folder}/{n:02}_{k}{src.suffix}','sha256':digest(src.read_bytes()),'role':'identity_master' if k in ['C01','C05'] else 'exact_audio_slice' if kind=='audio' else 'environment_or_prop' if k in ['S15_NEW','P_RUAN'] else 'user_selected_smile_expression_only_not_identity_master','upload':True})
  ...
  intro=f"# {bid} · {b['name']} · v1.6\n\n时长{b['duration']}秒；全片计划起点{b['global_start']}秒。\n\n"+('**真实唱者/嘴型时钟待核，正式演唱Prompt尚未签发。**\n' if song else '**人物母版+文字动作Prompt已备；不需要静态吻图，实际前片与生成视频仍待验收。**\n')
  ...
  with tempfile.TemporaryDirectory(prefix='ch04_v16_') as tmp:
   ...
   dest=out/f'CH04_{bid}_v16.zip'
   pending=dest.with_suffix('.zip.tmp')
 ...
 write(out/'CH04_v16_AUDIO_REVIEW.html',audio_review_page(audio));shutil.copyfile(REL/'DIRECTOR_AND_PRODUCTION_PACK.md',out/'CH04_v16_DIRECTOR.md')
 with zipfile.ZipFile(out/'CH04_v16_REFERENCES.zip','w',zipfile.ZIP_DEFLATED) as z:
  for k in ['C01','C05','V15_SMILE','S15_NEW','P_RUAN']:z.write(ROOT/a['refs'][k],k+Path(a['refs'][k]).suffix)
  z.write(REL/'PAN_HUI_IDENTITY_RULES.md','PAN_HUI_IDENTITY_RULES.md');z.write(REL/'REFERENCE_ROUTE.md','REFERENCE_ROUTE.md')
 book='# 第四回 v1.6 · 歌后四段可复制Prompt\n\n每段五图：图1孙母版、图2潘母版、图3潘单人笑意、图4巨松空景、图5阮。另加实际前片末4秒。每段复制一个正文；真实视频待生成验收。\n\n'
 for b in t['blocks'][5:]:book+=f"## {b['id']} · {b['duration']}秒\n\n```text\n{b['prompt']}\n```\n\n"
 write(out/'CH04_v16_POST_SONG_PROMPTS.md',book)
 js(out/'BUILD_RECEIPT_v16.json',results)
